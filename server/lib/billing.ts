...
import type { Persona, ProductCode } from '../../src/types/account';
...
export function generatePaymentReference() {
    return `dr_${Date.now()}_${crypto.randomUUID().replace(/-/g, '').slice(0, 16)}`;
}
...
function validateVerifiedPayment(transaction: PaymentTransactionRow, verification: PaystackVerifyResponse['data']) {
    const verifiedProductCode = verification.metadata?.productCode;
    const resolvedProductCode = verifiedProductCode && isProductCode(verifiedProductCode)
        ? verifiedProductCode
        : transaction.plan_code;

    if (resolvedProductCode !== transaction.plan_code) {
...
export async function reconcileVerifiedPayment(input: {
    ...
    validateVerifiedPayment(transaction, input.verification);
